...
def check_jugan12(message):
    for k, p_url in dic_class_url_12.items():
        result = ""
        ...
        try:
            ...
            result += '에러발생'
            # time.sleep(2)
            bot.send_message(message.chat.id, '에러발생!')

@bot.message_handler(commands=['jugan34'])
def check_jugan34(message):
    for k, p_url in dic_class_url_34.items():
        result = ""
        res = requests.get(p_url, verify=False) #ssl 인증서 관련 
        res.raise_for_status()  # 정보가 없으면 프로그램 종료
        soup = BeautifulSoup(res.text, "lxml")
        ...
        try:
            ...
            # time.sleep(2)
            bot.send_message(message.chat.id, '에러발생!')
@bot.message_handler(commands=['jugan56'])
def check_jugan56(message):
    for k, p_url in dic_class_url_56.items():
        result = ""
        res = requests.get(p_url, verify=False) #ssl 인증서 관련 
        res.raise_for_status()  # 정보가 없으면 프로그램 종료
        soup = BeautifulSoup(res.text, "lxml")
